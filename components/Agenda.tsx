        ...
        <div className="lg:col-span-1 space-y-6">
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            ...
            <div className="grid grid-cols-7 gap-1">
              {[...Array(31)].map((_, i) => (
                <button key={i} className={`h-8 w-8 flex items-center justify-center rounded-lg text-sm transition-colors ${i + 1 === 20 ? 'bg-emerald-600 text-white font-bold' : 'hover:bg-slate-100 text-slate-600'}`}>
                  {i + 1}
                </button>
              ))}
            </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="p-4 border-b border-slate-100 bg-slate-50 flex justify-between items-center">
              ...
              <span className="text-xs bg-emerald-100 text-emerald-700 px-2 py-1 rounded-full font-bold">4 Pacientes</span>
            </div>
              ...
              {appointments.map((apt) => (
                <div key={apt.id} className="p-6 flex items-center justify-between hover:bg-slate-50 transition-colors group">
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      <h4 className="font-bold text-slate-900 flex items-center gap-2 group-hover:text-emerald-600 transition-colors">
                        {apt.patientName}
                      </h4>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <button className="text-slate-400 hover:text-emerald-600 p-2 rounded-full hover:bg-emerald-50 transition-all">
                      <FileText size={18} />
                    </button>
                  </div>
                </div>
